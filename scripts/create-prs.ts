...
if (isClean) {
  ...
  if (demoPRs.length === 0) {
    ...
    for (const pr of demoPRs) {
      console.log(
        `Closing PR #${pr.number} (${pr.headRefName}) and deleting branch...`,
      );
...
for (const n of failSet) {
  if (n < 1 || n > count) {
    console.error(
      `--fail value ${n} is out of range (must be between 1 and ${count})`,
    );
    process.exit(1);
  }
}
...
console.log(
  `Creating ${count} PR(s)${
    failSet.size > 0
      ? `, with PR(s) [${[...failSet].join(", ")}] set to fail CI`
      : ""
  }.\n`,
);
...
for (let i = 1; i <= count; i++) {
  ...
  console.log(
    `[${i}/${count}] Creating branch ${branch}${
      isFailing ? " (WILL FAIL CI)" : ""
    }...`,
  );
  ...
  run("git add .");
  run(
    `git commit -m "demo: add entry ${num}${
      isFailing ? " [will fail CI]" : ""
    }"`,
  );
  ...
  if (existingPRs.length > 0) {
    console.log(
      ...
      } already exists for ${branch}, skipping creation`,
    );
  } else {
